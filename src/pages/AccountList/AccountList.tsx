...
export default function AccountList() {
  useDocumentTitle("Prepify | Danh sách tài khoản");
  ...
  const { data, isLoading } = useGetTableAccounts({
    sorting,
    columnFilters: debouncedColumnFilters,
    pagination,
  });

  const handleRenderToolbar = useCallback((table: Table<TableAccountType>) => <DataTableToolbar table={table} />, []);

  return (
    <LayoutBody className="flex flex-col" fixedHeight>
      <div className="mb-2 flex items-center justify-between space-y-2">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Danh sách tài khoản</h2>
          <p className="text-muted-foreground">Hiển thị danh sách tất cả tài khoản trên hệ thống.</p>
        </div>
      </div>
