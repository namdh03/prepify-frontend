...
import CheckoutSkeleton from "~pages/Checkout/components/CheckoutSkeleton";
...
const dummyData = {
  ...
  totalGoods: 234.78,
  totalPrice: 234.78,
  trackingNumber: "PREP0555621674628",
  payment: {
    id: "1",
    icon: icons.VNPAY,
    name: "VNPAY",
...
export default function UserPurchaseDetail() {
  ...
  const steps = useMemo(
    () =>
      [
        {
          label: `Đơn Hàng Đã Thanh Toán (đ${dummyData.totalPrice.toLocaleString()})`,
          icon: HandCoins,
          description: dayjs(dummyData.orderDate).format("HH:mm DD-MM-YYYY"),
        },
  ...
  return (
    <section>
        ...
        <div className="flex flex-col gap-2">
          <div>
            ...
            <Badge variant={"outline"} className="py-1">
              {dummyData.trackingNumber}
            </Badge>
          </div>

          <div>
            <span className="text-foreground text-sm">NGÀY ĐẶT HÀNG: </span>
            <span>{dayjs(dummyData.orderDate).format("DD/MM/YYYY")}</span>
          </div>
        </div>

        <div>
          {dummyData.status === OrderStatus.WAITING && <span className="text-primary">CHỜ XÁC NHẬN</span>}
          {dummyData.status === OrderStatus.CREATED && <span className="text-primary">ĐÃ XÁC NHẬN</span>}
          {dummyData.status === OrderStatus.PICKED_UP && <span className="text-primary">ĐÃ NHẬN ĐƠN</span>}
          {(dummyData.status === OrderStatus.DELIVERING || dummyData.status === OrderStatus.DELAYED) && (
            <span className="text-primary">ĐANG GIAO HÀNG</span>
          )}
          {dummyData.status === OrderStatus.DELIVERED && (
          ...
          {dummyData.status === OrderStatus.CANCELED && <span className="text-destructive">ĐÃ HỦY</span>}
        </div>
      </div>
      ...
      <Table className="bg-white text-[#09090B]">
        ...
        <TableBody>
          {isFetching
            ? Array.from({ length: LIMIT }).map((_, index) => <CheckoutSkeleton key={index} />)
            : dummyData.orderItems.map((item) => (
                    ...
                    <TableCell className="px-4 py-5">
                      <div className="flex items-center">
                        <Link to={`${configs.routes.shop}/${item.slug}`} className="flex items-center">
                          ...
                          <h4 className="w-44 ml-3 text-base font-normal leading-5 line-clamp-3 break-keep">
                            {item.name}
                          </h4>
                        </Link>
      ...
      {(initialStep !== 0 || dummyData.status === OrderStatus.WAITING) && (
        <>
          <Separator className="my-8" />

          <div className="bg-white">
            <div className="flex items-center justify-between">
              {dummyData.status !== OrderStatus.WAITING && (
                <div className="flex flex-col gap-3">
                  <h3 className="text-xl font-semibold leading-9">Phương thức thanh toán</h3>
                  <img
                    key={dummyData.payment.id}
                    src={dummyData.payment.icon}
                    alt=""
              ...
              <div className="ml-auto flex flex-col gap-6 min-w-[450px]">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-base font-normal leading-7">
                    <sup>₫</sup>
                    {dummyData.totalGoods.toLocaleString()}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-base font-normal leading-7">
                    <sup>₫</sup>
                    {dummyData.deliveryPrice.toLocaleString()}
                  </span>
                </div>

                <div className="flex items-center justify-between">
                  <span className="text-[#71717A] text-base font-normal leading-7">
                    Tổng thanh toán ({dummyData.orderItems.length || 0} sản phẩm):
                  </span>
                  <span className="text-primary text-xl font-semibold leading-7">
                    <sup>₫</sup>
                    {dummyData.totalPrice.toLocaleString()}
                  </span>
                </div>
                ...
                {dummyData.status !== OrderStatus.WAITING && (
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-base font-normal leading-7">{dummyData.payment.name}</span>
                  </div>
                )}

                {dummyData.status === OrderStatus.WAITING && <Button>THANH TOÁN</Button>}
              </div>
            </div>
          </div>
        </>
